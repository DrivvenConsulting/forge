...
REGISTRY_CATEGORIES: tuple[str, ...] = ("agents", "rules", "skills", "bundles", "workflows", "prompts", "hooks", "gh-actions")
...
def _parse_bundle_manifest(manifest_path: Path) -> BundleManifest | None:
    ...
    try:
        ...
        for ref in data["items"]:
            ...
            kind = ref.get("kind")
            item_id = ref.get("id")
            if kind not in ("agent", "rule", "skill", "workflow", "prompt", "hook", "gh-action") or not item_id:
                continue
