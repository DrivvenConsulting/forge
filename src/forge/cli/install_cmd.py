...
def install_cmd(
    ...
    if kind == "bundle":
        ...
        if not is_compatible_with_project_type(bundle_item, config.project_type):
            typer.echo(f"Bundle {item_id} is not compatible with project type {config.project_type}.", err=True)
            raise typer.Exit(1)
        install_bundle(
            registry_root,
            bundle_item,
        ...
        if key not in by_kind_id:
            ...
            raise typer.Exit(1)
        item = by_kind_id[key]
        if not is_compatible_with_project_type(item, config.project_type):
            typer.echo(f"{kind}/{item_id} is not compatible with project type {config.project_type}.", err=True)
            raise typer.Exit(1)
