...
from forge.core.validation import (
...
def test_is_compatible_with_project_types() -> None:
    item = RegistryItem(
        kind="rule",
        id="r1",
        version="1.0.0",
    ...
    assert is_compatible_with_project_types(item, ["backend"]) is True
    assert is_compatible_with_project_types(item, ["data"]) is True
    assert is_compatible_with_project_types(item, ["backend", "infra"]) is True
    assert is_compatible_with_project_types(item, ["frontend", "infra"]) is False
